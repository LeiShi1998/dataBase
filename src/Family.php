...
<?php
include("conn.php");
$sql="select familyName,familySpNumALL,msa from family";
...
<div class="content">
  ...
  <?php
  $column=["Family","Selenoprotein Number","Multiple sequence alignment"];
  ?>
  ...
  while($array=mysqli_fetch_row($res))
  {
      ...
      foreach ($array as $data)
      {
        if($flag==0)
        {
          echo "<td><a class='textUrl' href='Search.php?options=Family&pname=$data'>".$data."</td>";
          $flag=1;
        }
        else
          echo "<td>".$data."</td>";
